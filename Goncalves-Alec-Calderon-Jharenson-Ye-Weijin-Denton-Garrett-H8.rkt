...
(check-expect(eval "let x = 2 in *(2, 8)") (num-val 16))

(check-expect(eval "zero?(-(2, 1))") (bool-val #f))

(check-expect(eval "if zero?(-(2, 1)) then x else 2") (num-val 2))

(check-expect(eval "letrec fact(n) = if zero?(n) then 1 else *(n, (fact -(n, 1)))
       in (fact 5)") (num-val 120))

(check-expect (eval "let x = 1 y = 2 in -(y,x)") (num-val 1))

(check-expect (eval "let x = 5 y = 10 z = 15 in +(x, -(y, z))") (num-val 0))

(check-expect (eval "let x = 1 y = 2 z =3 in let a = 4 b = 5 in *(y, b)") (num-val 10))

(check-expect (eval "let x = 1 y = 2 z = 3
                       in let a = +(x, y) b = *(y, z)
                            in +(a, b)") (num-val 9))

(check-expect (eval "let x = 10 y = 12 z = 15
